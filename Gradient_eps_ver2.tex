...
While the solution to any second-order linear ODE can be written as a superposition of two independent solutions, 
From the matrix structure in system (\ref{eqns:BC_mat_sys}) we can deduce that the  inner-layer solution must be written as a superposition of two independent solutions. Writing the solution as a sum of left and right travelling waves makes the physical meaning of the solution more tractable. As we shall see next, the approximation methods applied in this work - regular perturbations and WKB - have both properties.\\
Further, both the matrix and the input vector are complex valued. Hence we shall typically have a complex-valued solution vector. The computed components $E_r$, $E_L$, $E_R$, $E_t$ therefore yield both amplitude and phase information.   
